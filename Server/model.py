...
class CarBot(ap.Agent):
    ...
    def find_new_cell(self, road, intersections):
        ...
        if (i,j) in intersections:
            self.handle_intersection(intersections[(i,j)])
        # move to the right
        if self.direction == "right":
            # check if road is open
            if (i,j+1) in road and (i,j+1) in self.grid.empty and ((i,j+2) not in road or (i,j+2) in self.grid.empty):
        ...
        # move down
            ...
            # check if road is open
            if (i+1,j) in road and (i+1,j) in self.grid.empty and ((i+2,j) not in road or (i+2,j) in self.grid.empty):
...
#
    ...
    def step(self):
        ...
        for a in self.grid.agents:
            a.find_new_cell(self.street_coords, self.intersections)
